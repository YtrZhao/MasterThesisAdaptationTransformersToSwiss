...
def main():
    ...
    gbert_models = [
        "deepset/gbert-base",
        "GBert/MLM_archimob_subset",
        "GBert/MLM_swiss_crawl_64k",
        "GBert/MLM_swiss_crawl_full_new",
    ]
    ...
    for model_name in gbert_models:
        for adapter_config in adapter_configs:
            config = BertConfig.from_pretrained(model_name, num_labels=4)
            model = BertModelWithHeads.from_pretrained(model_name, config=config)
            ...
            training_args = TrainingArguments(
                output_dir=f"./gbert_base_adapter/{model_name}_{adapter_name}",
                do_train=True,
                do_eval=True,
                num_train_epochs=6,
                learning_rate=1e-4,
